...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    const uniqueSuffix =
      Date.now() + "-theQuoteByAtif-" + Math.round(Math.random() * 1e9);
...
const uploadToCloud = (filePath) => {
  return new Promise((resolve, reject) => {
    cloudinary.v2.uploader.upload(filePath, (err, url) => {
...
const uploadFromURL = (image_url) => {
  return new Promise((resolve, reject) => {
    cloudinary.v2.uploader.upload(image_url, (err, url) => {
      if (err) return reject(err);
      return resolve(url);
    });
  });
...
module.exports.destroyFile = destroyFile;
