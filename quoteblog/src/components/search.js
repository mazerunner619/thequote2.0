...
export default function Search({match}) {
    ...
    setLoading(true)
      const getData = async() => {
        const searchFor = match.params.what ? match.params.what : ""; 
          const {data} = await axios.get(`/search/${searchFor}/${loggedUser._id}`);
  ...
  const resultArr = result.map( res =>   
    ...
    <div style={{float : "right",display : "inline", fontSize : "150%"}} >
    {
            ...
            : 
            (!res.Friends &&
              <TiUserAdd style={{color: "black",cursor :"pointer", fontSize : "120%"}} onClick = {(e) => sendFriendRequest(e,res._id)}/>
              )
          }
    </div>
...
return (
    <div id="fullPage">
      ...
      (
        ...
        :
        <b style={{textAlign:"center"}}><i>no results found for {match.params.what}</i></b>
      )
}

<p style={{ border : "1px solid #97124368", fontSize : "200%", textAlign : "center", color : "black", fontFamily : "fantasy"}}><i>Discover...</i></p>
