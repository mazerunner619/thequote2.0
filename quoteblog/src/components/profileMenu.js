...
export default function ScrollableTabsButtonPrevent({ userid, profile }) {
        ...
        <Card.Body>
          <div style={{ float: "left" }} className="mr-2">
            {res.from.profilePicture && res.from.profilePicture.imageURL ? (
              <Card.Img
                src={res.from.profilePicture.imageURL}
                roundedCircle
                id="post-uploader"
                alt="profile-picture"
              />
          ...
          <Card.Title
            ...
            onClick={() => hist.push(`/show/${res.from._id}/profile`)}
          >
            {res.from.username}
          </Card.Title>
          <Card.Text className="text-muted">{res.from.bio}</Card.Text>
        </Card.Body>
        <Row className="ml-0 mr-0">
          <Col className="m-0 p-0">
            <Button
              onClick={(e) => handleAcceptRequest(res._id)}
              style={{ width: "100%" }}
              color="secondary"
              variant="contained"
            >
          ...
          <Col className="m-0 p-0">
            <Button
              onClick={(e) => handleDeleteRequest(res._id)}
              style={{ width: "100%" }}
              variant="outlined"
              color="success"
            >
            ...
            <Dropdown>
              ...
              <Dropdown.Menu>
                <Dropdown.Item
                  ...
                  onClick={async () => {
                    await axios.post(
                      `/user/${loggedUser._id}/unfriend/${res._id}`
                    );
                    setRender(render === 0 ? 1 : 0);
                  }}
                >
  ...
  const myPostsArr = POSTS.map((x) => (
    <Col key={x._id} sm={4} md={3} lg={3} xs={4} className="p-0">
          ...
          <TabPanel value={value} index={0}>
            <Row id="myuploads">
              {POSTS.length > 0 ? (
                ...
                <b style={{ textAlign: "center", color: "#b6416c" }}>
                  <i>nothing to show</i>
                </b>
              )}
            </Row>
