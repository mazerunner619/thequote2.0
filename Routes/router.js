...
router.get("/getfriends/:userid", async (req, res) => {
  try {
    ...
    const user = await db.Client.findById(userid).populate({
      path: "friends",
      select: "_id username active profilePicture",
    });
    ...
    let CHATS = await db.Chat.find(
      {
        recepients: { $elemMatch: { $eq: userid.toString() } },
      },
      {
        chats: { $slice: -1 },
      }
    ...
    CHATS.forEach((chat) => {
      const indexOfFriend = 1 - chat.recepients.indexOf(userid);
      if (chat.recepients[indexOfFriend]) {
        const lastMsg = chat.chats.length > 0 ? chat.chats[0].content : "";
        friendMap.set(chat.recepients[indexOfFriend].toString(), lastMsg);
      }
    });
    ...
    const result = user.friends.map((F) => {
      const obj = {
        ...
        active: F.active,
        lastMessage: friendMap.get(F._id.toString()),
      };
      return obj;
    });
    res.send(result);
  } catch (err) {
    console.log("errorr here", err.message, err);
  }
});
...
router.get("/search/:username/:userid", async (req, res, next) => {
  try {
    ...
    const requser = await db.Client.findById(userid).populate({
      path: "sentRequests",
    });
    ...
    let searching = user.filter((info) => {
      ...
      return (
        uname.indexOf(username.toLowerCase()) === 0 &&
        requser._id.toString() !== info._id.toString()
      );
    });
    ...
    let all = user.filter((info) => {
      const uname = info.username.toLowerCase();
      return (
        uname.indexOf(username.toLowerCase()) === -1 &&
        requser._id.toString() !== info._id.toString()
      );
    });

    all = all.map((obj) => {
      const requestSent = requser.sentRequests.filter(
        (SR) => SR.to.toString() === obj._id.toString()
      );
      const alreadyFriends = obj.friends.filter(
        (FRND) => FRND.toString() === requser._id.toString()
      );
...
router.get("/getuser/:userID", async (req, res, next) => {
  try {
    const user = await db.Client.findById(req.params.userID).populate({
      path: "posts friends receivedRequests",
      populate: { path: "from" },
    });
...
router.get("/current", async (req, res, next) => {
  try {
    ...
    if (token) {
      ...
      if (verified) {
        const user = await db.Client.findById(verified.userId).populate({
          path: "posts receivedRequests friends",
          populate: { path: "from" },
        });
        res.send(user);
      } else {
...
router.get("/getmyposts/:id", async (req, res, next) => {
  try {
    const user = await db.Client.findById(req.params.id).populate("posts");
